...
describe RingBuffer do
  ...
  it "correctly handles a mix of pushes/pops and shifts/unshifts" do
    ...
    4.times do |idx|
      expect(arr[idx]).to eq(3-idx)
      expect(arr[idx+4]).to eq(idx)
    end

    3.downto(0) do |idx|
      expect(arr.shift).to eq(idx)
      expect(arr.pop).to eq(idx)
    end
  end
  ...
  it "correctly handles pushes/pops/shifts/unshifts after resizing" do
    ...
    5.times do |idx|
      expect(arr[idx]).to eq(4-idx)
      expect(arr[idx+5]).to eq(idx)
    end
